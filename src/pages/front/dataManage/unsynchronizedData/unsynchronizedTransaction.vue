...
import { findNotSyncInventoryTransactionList, exportNotSyncInventoryTransactionList } from '@/api/front/unsynchronized'
export default {
  name: 'unsynchronizedTransaction',
  data () {
    return {
      skuCode: '',
      totalSize: 8,
  ...
  methods: {
    ...
    fetchDataList (data) {
      findNotSyncInventoryTransactionList(data).then(res => {
        if (res.data.code === 200) {
          const data = res.data.data
          this.totalSize = data.totalSize
          this.tableData = data.notSyncInventoryTransactionVOList
